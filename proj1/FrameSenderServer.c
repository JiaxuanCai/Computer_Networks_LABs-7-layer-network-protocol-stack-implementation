...
unsigned char buf[1536]; // maximum size of the frame is 1536 bytes.
unsigned char data[] = "Hello? Hello World, I'm the message to be encapsulated and sent to another address, following the protocol of IEEE 802.3 which is the Ethernet. I really love Computer Network, I really love programming in C-language. Good luck!!\n";

int main() {
    //encapsulate the frame.
	frame_encapsulate(&destination_mac,&source_mac,0x86dd,data,sizeof(data),&buf[0]);

	//send the frame.
	send_frame(buf,sizeof(data)+18);
	return 0;
}
