...
mac_addr local_mac = {0x8d,0x17,0x46,0x41,0x0e,0xad};

//crc32tab, defined for CRC32 check.
...
/*
    Generate the CRC32 check sequence based on an input sequence.
    input:
        unsigned char *buf: the sequence used to generate the CRC sequence
        unsigned int size: size of the sequence
    output:
        int: crc32 sequence generated
*/
...
int main()
{
	...
	local.sin_port = htons(SERVER_PORT); // listening port
	...
	printf("This device's MAC address is 8D-17-46-41-0E-AD\n");
	unsigned char bufferIn[10240];
	while(1)
	{
		bufferIn[0] = '\0';
		...
		int frameSize=0;
		if((frameSize=recvfrom(sock_fd, bufferIn, sizeof(bufferIn), 0, (struct sockaddr *)&from, &len)) != SOCKET_ERROR)
		{
			//1.
            ...
            //be generated.
			unsigned int crc32_result = crc32(bufferIn,frameSize-4);
			unsigned char crc32_result_c[4];
			memcpy(crc32_result_c,&crc32_result,sizeof(crc32_result));
			if (
				crc32_result_c[3] != bufferIn[frameSize-1] ||
				crc32_result_c[2] != bufferIn[frameSize-2] ||
				crc32_result_c[1] != bufferIn[frameSize-3] ||
				crc32_result_c[0] != bufferIn[frameSize-4]
			) {
